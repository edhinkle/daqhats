/*****************************************************************************
    ...
    Adapted from finite_scan example in original daqhats example directory 
    for mcc172 for use within precision laser inclinometer.
...
double calc_dely(double* data, uint8_t num_channels,
		 uint32_t num_samples_per_channel)
{
  double channel_rms[num_channels];
  uint8_t i; 
  
  for (i=0; i<num_channels; i++) {

    channel_rms[i] = calc_rms(data, i, num_channels,
  ...
  dely = (channel_rms[0] - channel_rms[1]) / (channel_rms[0] +
					      channel_rms[1]);
...
int main(void)
{
    ...
    uint32_t samples_per_channel = 200; // S/s
    ...
    double scan_rate = 400.0; // Total number of samples over all channels (?)
    ...
    strcat(display_header, "Y-Offset");
    ...
    do
    {
	    ...
	    printf("%10.4f", calc_dely(read_buf, num_channels,
				  samples_read_per_channel));
            fflush(stdout);
